...
import AddReviewErrorMessages from '../components/forms/AddReviewErrorMessages.jsx';
import AddReviewUserImages from '../components/forms/AddReviewUserImages.jsx';
import AddStarRating from '../utils/AddStarRating.jsx';
import {
	convertFile,
	convertFilesToDataURL,
	postRequirements,
} from '../lib/formUtilityFunctions.js';

describe('Utility functions', () => {
	test('Correctly converts files to strings', async () => {
		const mockSingleFile = new File(['test'], 'test.png', {
			type: 'image/png',
		});
		const mockFiles = {
...
describe('Add review form', () => {
	...
	test('Correctly displays the product passed in', async () => {
		...
		for (let i = 0; i < characteristicsList.length; i++) {
			expect(
				await screen.findByText(characteristicsList[i])
			).toBeInTheDocument();
		}
	});
	...
	test('Correctly displays error messages', async () => {
		...
		const { container } = await render(
			<AddReviewErrorMessages messages={mockErrors} />
		);
	...
	describe('Form Inputs', () => {
		...
		test('Summary input updates correctly', async () => {
			const summaryInput = await screen.findByPlaceholderText(
