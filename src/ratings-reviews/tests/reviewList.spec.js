...
import ReviewItem from '../components/review-item/ReviewItem.jsx';
...
describe('Review Item', () => {
	test('Correctly displays review information', async () => {
		for (let i = 0; i < reviews.count; i++) {
			...
			recommend &&
				expect(
					await screen.findByText('I recommend this product')
				).toBeInTheDocument();
...
describe('List of reviews', () => {
	...
	test('Displays the correct number of reviews', async () => {
		const { rerender } = render(
			await (<ReviewList reviewList={reviews.results} />)
		);
		...
		expect(
			await screen.findByText('Whoops there are no reviews here!')
		).toBeInTheDocument();
	});
